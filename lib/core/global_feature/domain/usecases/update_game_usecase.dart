...
class UpdateGameUseCase implements UseCase<ChessGameEntity, UpdateGameParams> {
  ...
  Future<Either<Failure, ChessGameEntity>> call(UpdateGameParams params) async {
    try {
      AppLogger.info('Updating game: ${params.game.uuid}', tag: 'UpdateGameUseCase');

      final result = await repository.updateGame(params.game);

      result.fold(
        (failure) =>
            AppLogger.error('Failed to update game: ${failure.message}', tag: 'UpdateGameUseCase'),
        (game) {
          AppLogger.info('Game updated successfully: ${game.uuid}', tag: 'UpdateGameUseCase');
          AppLogger.gameEvent(
            'GameUpdated',
            data: {'uuid': game.uuid, 'movesCount': game.movesCount, 'result': game.result},
          );
        },
      );
      ...
      return Left(DatabaseFailure(message: 'Failed to update game: ${e.toString()}'));
    }
  }
}

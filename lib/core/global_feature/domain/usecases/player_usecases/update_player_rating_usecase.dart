...
class UpdatePlayerRatingUseCase implements UseCase<PlayerEntity, UpdatePlayerRatingParams> {
  final PlayerRepository repository;

  UpdatePlayerRatingUseCase(this.repository);

  @override
  Future<Either<Failure, PlayerEntity>> call(UpdatePlayerRatingParams params) async {
    try {
      AppLogger.info(
      ...
      if (params.newRating < 0 || params.newRating > 3500) {
        return Left(ValidationFailure(message: 'Invalid rating value: ${params.newRating}'));
      }

      final result = await repository.updatePlayerRating(params.playerUuid, params.newRating);
      ...
      return Left(DatabaseFailure(message: 'Failed to update player rating: ${e.toString()}'));
    }
  }
}
...
class UpdatePlayerRatingParams extends Equatable {
  ...
  const UpdatePlayerRatingParams({required this.playerUuid, required this.newRating});

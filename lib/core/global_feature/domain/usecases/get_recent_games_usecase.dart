...
class GetRecentGamesUseCase implements UseCase<List<ChessGameEntity>, GetRecentGamesParams> {
  final ChessGameRepository repository;

  GetRecentGamesUseCase(this.repository);

  @override
  Future<Either<Failure, List<ChessGameEntity>>> call(GetRecentGamesParams params) async {
    try {
      AppLogger.info(
      ...
      result.fold(
        ...
        (games) =>
            AppLogger.info('Fetched ${games.length} recent games', tag: 'GetRecentGamesUseCase'),
      );
      ...
      return Left(DatabaseFailure(message: 'Failed to fetch recent games: ${e.toString()}'));
    }
  }
}

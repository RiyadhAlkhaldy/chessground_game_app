...
class GameService {
  ...
  static Either<Failure, bool> validateMove(GameState gameState, Move move) {
    try {
      ...
      if (!isLegal) {
        AppLogger.warning('Illegal move attempted: ${move.uci}', tag: 'GameService');
      ...
      return Left(GameStateFailure(message: 'Failed to validate move: ${e.toString()}'));
    }
  }
  ...
  static Either<Failure, GameState> executeMove(
    ...
    try {
      ...
      AppLogger.error('Error executing move', error: e, stackTrace: stackTrace, tag: 'GameService');
      return Left(GameStateFailure(message: 'Failed to execute move: ${e.toString()}'));
    }
  }

  /// Check if game should end and determine result
  /// التحقق من ضرورة انتهاء اللعبة وتحديد النتيجة
  static Either<Failure, GameTermination> checkGameTermination(GameState gameState) {
    try {
      ...
      return Left(GameStateFailure(message: 'Failed to check game termination: ${e.toString()}'));
    }
  }

  /// Calculate game result string based on termination
  /// حساب نص نتيجة اللعبة بناءً على نوع الإنهاء
  static String calculateResult(GameState gameState, GameTermination termination) {
  ...
  static Either<Failure, ChessGameEntity> syncGameStateToEntity(
    ...
    try {
      ...
      final gameStateEntity = GameStateConverter.toEntity(gameState, existingGame.uuid);
      ...
      AppLogger.debug('Successfully synced GameState to Entity', tag: 'GameService');
      ...
      return Left(GameStateFailure(message: 'Failed to sync game state: ${e.toString()}'));
    }
  }

  /// Restore GameState from ChessGameEntity
  /// استعادة GameState من ChessGameEntity
  static Either<Failure, GameState> restoreGameStateFromEntity(ChessGameEntity game) {
    try {
      AppLogger.debug('Restoring GameState from ChessGameEntity: ${game.uuid}', tag: 'GameService');
      ...
      for (final moveEntity in game.moves) {
        if (moveEntity.lan != null) {
          ...
          gameState.play(move!, comment: moveEntity.comment, nags: moveEntity.nags);
        }
      }
      ...
      return Left(GameStateFailure(message: 'Failed to restore game state: ${e.toString()}'));
    }
  }
  ...
  static Either<Failure, Move> parseMoveFromUci(GameState gameState, String uci) {
    try {
  ...
  static Either<Failure, Move> parseMoveFromSan(GameState gameState, String san) {
    try {

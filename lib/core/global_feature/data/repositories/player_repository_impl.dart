...
class PlayerRepositoryImpl implements PlayerRepository {
  ...
  Future<Either<Failure, PlayerEntity>> savePlayer(PlayerEntity player) async {
    try {
      AppLogger.info('Repository: Saving player ${player.name}', tag: 'PlayerRepository');

      final model = player.toModel();
      final savedModel = await localDataSource.savePlayer(model);
      final savedEntity = savedModel.toEntity();

      AppLogger.info('Repository: Player saved successfully', tag: 'PlayerRepository');
      ...
      return Left(DatabaseFailure(message: 'Failed to save player: ${e.toString()}', details: e));
    }
  }
  ...
  Future<Either<Failure, PlayerEntity>> updatePlayer(PlayerEntity player) async {
    try {
      AppLogger.info('Repository: Updating player ${player.name}', tag: 'PlayerRepository');

      final model = player.toModel();
      final updatedModel = await localDataSource.updatePlayer(model);
      final updatedEntity = updatedModel.toEntity();

      AppLogger.info('Repository: Player updated successfully', tag: 'PlayerRepository');
      ...
      return Left(DatabaseFailure(message: 'Failed to update player: ${e.toString()}', details: e));
    }
  }
  ...
  Future<Either<Failure, PlayerEntity>> getPlayerByUuid(String uuid) async {
    try {
      AppLogger.info('Repository: Fetching player $uuid', tag: 'PlayerRepository');

      final model = await localDataSource.getPlayerByUuid(uuid);
      final entity = model.toEntity();

      AppLogger.info('Repository: Player fetched successfully', tag: 'PlayerRepository');
      ...
      if (e.toString().contains('not found')) {
        return Left(NotFoundFailure(message: 'Player not found with UUID: $uuid'));
      }

      return Left(DatabaseFailure(message: 'Failed to fetch player: ${e.toString()}', details: e));
    }
  }
  ...
  Future<Either<Failure, PlayerEntity>> getPlayerById(int id) async {
    try {
      AppLogger.info('Repository: Fetching player by ID $id', tag: 'PlayerRepository');
  ...
  Future<Either<Failure, List<PlayerEntity>>> getAllPlayers() async {
    try {
      AppLogger.info('Repository: Fetching all players', tag: 'PlayerRepository');

      final models = await localDataSource.getAllPlayers();
      final entities = models.map((m) => m.toEntity()).toList();

      AppLogger.info('Repository: Fetched ${entities.length} players', tag: 'PlayerRepository');
      ...
      return Left(DatabaseFailure(message: 'Failed to fetch players: ${e.toString()}', details: e));
    }
  }

  @override
  Future<Either<Failure, List<PlayerEntity>>> getPlayersByType(String type) async {
    try {
      AppLogger.info('Repository: Fetching players by type $type', tag: 'PlayerRepository');
      ...
      return Left(
        DatabaseFailure(message: 'Failed to fetch players by type: ${e.toString()}', details: e),
      );
    }
  }
  ...
  Future<Either<Failure, bool>> deletePlayer(String uuid) async {
    try {
      AppLogger.info('Repository: Deleting player $uuid', tag: 'PlayerRepository');
      ...
      return Left(DatabaseFailure(message: 'Failed to delete player: ${e.toString()}', details: e));
    }
  }

  @override
  Future<Either<Failure, List<PlayerEntity>>> searchPlayersByName(String name) async {
    try {
      AppLogger.info('Repository: Searching players by name: $name', tag: 'PlayerRepository');

      final models = await localDataSource.searchPlayersByName(name);
      final entities = models.map((m) => m.toEntity()).toList();

      AppLogger.info('Repository: Found ${entities.length} players', tag: 'PlayerRepository');
      ...
      return Left(
        DatabaseFailure(message: 'Failed to search players: ${e.toString()}', details: e),
      );
    }
  }

  @override
  Future<Either<Failure, PlayerEntity>> updatePlayerRating(String uuid, int newRating) async {
    try {
      AppLogger.info(
      ...
      AppLogger.info('Repository: Player rating updated successfully', tag: 'PlayerRepository');
      ...
      return Left(
        DatabaseFailure(message: 'Failed to update player rating: ${e.toString()}', details: e),
      );
    }
  }

  @override
  Future<Either<Failure, PlayerEntity>> getOrCreateGuestPlayer(String name) async {
    try {
      AppLogger.info(
      ...
      return Left(
        DatabaseFailure(message: 'Failed to get/create guest player: ${e.toString()}', details: e),
      );
    }
  }
  ...
  Stream<Either<Failure, PlayerEntity>> watchPlayer(String uuid) {
    try {
      AppLogger.info('Repository: Watching player $uuid', tag: 'PlayerRepository');
      ...
      return Stream.value(
        Left(DatabaseFailure(message: 'Failed to watch player: ${e.toString()}')),
      );
    }
  }
}

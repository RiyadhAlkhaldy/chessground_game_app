...
class GameStateCacheDataSourceImpl implements GameStateCacheDataSource {
  ...
  Future<void> cacheGameState(GameStateModel state) async {
    try {
      AppLogger.debug('Caching game state: ${state.gameUuid}', tag: 'GameStateCacheDataSource');
  ...
  Future<GameStateModel> getCachedGameState(String gameUuid) async {
    try {
      AppLogger.debug('Retrieving cached game state: $gameUuid', tag: 'GameStateCacheDataSource');
      ...
      AppLogger.debug('Game state retrieved from cache', tag: 'GameStateCacheDataSource');
  ...
  Future<void> removeCachedGameState(String gameUuid) async {
    try {
      AppLogger.debug('Removing cached game state: $gameUuid', tag: 'GameStateCacheDataSource');
  ...
  Future<Map<String, GameStateModel>> getAllActiveStates() async {
    try {
      AppLogger.debug('Retrieving all active game states', tag: 'GameStateCacheDataSource');
  ...
  Future<void> clearAllCachedStates() async {
    try {
      AppLogger.debug('Clearing all cached game states', tag: 'GameStateCacheDataSource');

      final count = _cache.length;
      _cache.clear();

      AppLogger.debug('Cleared $count cached game states', tag: 'GameStateCacheDataSource');
    } catch (e, stackTrace) {
      AppLogger.error(

...
class SavePlayerUseCase implements UseCase<PlayerEntity, SavePlayerParams> {
  ...
  Future<Either<Failure, PlayerEntity>> call(SavePlayerParams params) async {
    try {
      AppLogger.info(
        'Saving player: ${params.player.name}',
      ...
      if (validation != null) {
        AppLogger.error(
          'Player validation failed: $validation',
          tag: 'SavePlayerUseCase',
        );
      ...
      result.fold(
        ...
        (player) => AppLogger.info(
          'Player saved successfully: ${player.name}',
          tag: 'SavePlayerUseCase',
        ),
      );
      ...
      return Left(
        DatabaseFailure(message: 'Failed to save player: ${e.toString()}'),
      );
    }
  }
